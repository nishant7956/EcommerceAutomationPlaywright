...
using OrangeHRM.Playwright.Framework.Browser;
using OrangeHRM.Playwright.Framework.Config;

namespace OrangeHRM.Playwright.Tests.Hooks;
...
public abstract class BaseUiTest
{
    ...
    /// <summary>
    /// Logs in using the admin credentials defined in appsettings.json.
    /// Returns the DashboardPage representing the resulting state.
    /// </summary>
    protected async Task<OrangeHRM.Playwright.Framework.Pages.DashboardPage> LoginAsAdminAsync()
    {
        var admin = Settings.AdminUser;
        var loginPage = new OrangeHRM.Playwright.Framework.Pages.LoginPage(Page, Settings);
        
        await loginPage.OpenAsync();
        return await loginPage.LoginAsAsync(admin.Username, admin.Password);
    }
}
